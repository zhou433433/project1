...
(function () {
    ...
    function init() {
        document.querySelector('#login-form-btn').addEventListener('click', onSessionInvalid); //加监听器，back to login
        ...
        validateSession(); //看用户是不是登录过
    }


    function validateSession() {
        onSessionInvalid(); //没登录过
        var url = './login';
        var req = JSON.stringify({}); //login servlet
    ...
    function onSessionInvalid() {
        ...
        hideElement(itemNav); //隐藏没登录用户不该看见的东西
        hideElement(itemList);
        hideElement(avatar); //小像
        hideElement(logoutBtn);
        hideElement(welcomeMsg);
        hideElement(registerForm);

        clearLoginError(); //清除之前可能的错误信息
    ...
    function register() {
        ...
        password = md5(username + md5(password)); //md5 - encode（编码）

        // The request parameters
        var url = './register'; //对应registerServlet
        ...
        ajax('POST', url, req,
            // successful callback
            function(res) { //request成功
                var result = JSON.parse(res);

                // successfully logged in
                if (result.status === 'OK') { //注册成功
    ...
    function ajax(method, url, data, successCallback, errorCallback) { //不停止主线程 要数据
        var xhr = new XMLHttpRequest(); //ajax call

        xhr.open(method, url, true); //method = GET/POST...

        xhr.onload = function() { //回调
            if (xhr.status === 200) {
    ...
    function hideElement(element) {
        element.style.display = 'none'; //inline, 优先级最高（除了!important）
    }
    ...
    function initGeoLocation() { //获取用户地理信息
